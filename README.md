...
在standAlone目录下，我们提供了两个脚本用于方便地启动和重启项目：

1. **启动项目**
```bash
cd /home/vanadium/HOJ/hoj-deploy/standAlone
./start.sh
```
...
- **start.sh**：使用 `docker-compose up -d` 命令启动所有容器，并输出访问地址
- **restart.sh**：先使用 `docker-compose down` 停止并移除所有容器，然后再使用 `docker-compose up -d` 启动项目

### 访问地址

- 前端：http://localhost:8081
- 后端API：http://localhost:6688
- 判题服务器：http://localhost:8090
